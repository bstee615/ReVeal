...
def do_one(datum):
    (idx, filename, code, applied), (tmpdir, raw_code_dir, all_parsed_dir, joern_parse) = datum
    ...
    proc = subprocess.run(f'bash {joern_parse} {tmp_dir.absolute()} -outdir {parsed_dir.absolute()}', shell=True,
...
def main(cmd_args=None):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with Pool(args.nproc) as p:
            ...
            pbar = tqdm.tqdm(p.imap_unordered(do_one, [(d, (tmpdir, raw_code_dir, all_parsed_dir, joern_parse)) for d in data], chunksize=10), total=len(data))
