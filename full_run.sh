...
mkdir -p "$outpdir/$name/data/full_experiment_real_data/$name";
...
python data_processing/create_ggnn_input.py --project "$name" --input "$inpdir" --output "$outpdir/$name/data/";

python data_processing/extract_slices.py --project "$name" --input "$inpdir" --text_in "$outpdir/$name/data/" --output "$outpdir/$name/data/";

python data_processing/create_ggnn_data.py --input "$outpdir/$name/data/" --project "$name" --csv "$inpdir/parsed/" --src "$inpdir/raw_code/" --wv "data/chrome_debian/raw_code_deb_chro.100" --output "$outpdir/$name/data/full_experiment_real_data/$name/$name.json";

python data_processing/full_data_prep_script.py --project "$name" --input "$inpdir" --base "$outpdir/$name/data/full_experiment_real_data/" --output "$outpdir/$name/data/full_experiment_real_data_processed";
